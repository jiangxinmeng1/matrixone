...
为了减少延迟，提交队列的pipelie中，worker1准备好日志后，异步调用Backend的Append接口，不等待Append成功，直接把事务交给下一个worker，然后开始处理下一个事务。目前有两种Backend，本地的Batch Store和分布式的Log Service。支持并发写入的Backend，能进一步加快提交。
