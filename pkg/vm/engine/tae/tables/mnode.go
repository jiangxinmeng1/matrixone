...
func (node *memoryNode) getDataWindowOnWriteSchema(
	...
	if ok {
		dest.Extend(node.data.CloneWindowWithBitmap(rowMask, mp))
		dest.GetVectorByName(objectio.TombstoneAttr_CommitTs_Attr).Extend(commitTSVec)
		dest.GetVectorByName(objectio.TombstoneAttr_Abort_Attr).Extend(abortVec)
		commitTSVec.Close()
		abortVec.Close()
	} else {
		inner := node.data.CloneWindowWithBitmap(rowMask, mp)
		...
		inner.AddVector(objectio.TombstoneAttr_Abort_Attr, abortVec)
		batWithVer.Seqnums = append(batWithVer.Seqnums, objectio.SEQNUM_COMMITTS, objectio.SEQNUM_ABORT)
...
func (node *memoryNode) CollectObjectTombstoneInRange(
	...
	abortIdx := 0
	rowMask.Foreach(func(row uint64) bool {
		if abortVec.Get(abortIdx).(bool) {
			abortIdx++
			return true
		}
		i := uint32(row)
		if types.PrefixCompare(rowIDs[i][:], objID[:]) == 0 {
			...
			(*bat).GetVectorByName(objectio.TombstoneAttr_CommitTs_Attr).Append(commitTSs[abortIdx], false)
		}
		abortIdx++
		return true
	})
