...
func TestAddBlksWithMetaLoc(t *testing.T) {
	...
	//read new non-appendable block data and check
	{
		...
		view1, err := blk1.GetColumnDataById(context.Background(), 2, true)
		assert.NoError(t, err)
		defer view1.Close()
		assert.True(t, view1.GetData().Equals(bats[0].Vecs[2]))
		assert.Equal(t, blk1.Rows(), bats[0].Vecs[2].Length())

		view2, err := blk2.GetColumnDataById(context.Background(), 2, true)
		assert.NoError(t, err)
		defer view2.Close()
...

	{
		...
		testutil.ForEachBlock(rel, func(blk handle.Block) (err error) {
			if blk.IsAppendableBlock() {
				view, err := blk.GetColumnDataById(context.Background(), 3, true)
				assert.NoError(t, err)
				defer view.Close()
				cntOfAblk++
				return nil
			...
			if bytes.Equal(metaLoc, metaLoc1) {
				view, err := blk.GetColumnDataById(context.Background(), 2, true)
				assert.NoError(t, err)
				defer view.Close()
...
func TestCompactMemAlter(t *testing.T) {
	...
	}
	{
		...
		assert.Nil(t, err)
		for i := 0; i <= 5; i++ {
			view, err := blk.GetColumnDataById(context.Background(), i, true)
			assert.NoError(t, err)
			defer view.Close()
			if i < 5 {
...
func TestFlushTabletail(t *testing.T) {
	...
	tae.Restart(ctx)
	{
		...
		for i := 0; it.Valid(); it.Next() {
			blk := it.GetBlock()
			views, err := blk.GetColumnDataByIds(context.Background(), idxs, true)
			require.NoError(t, err)
			defer views.Close()
...
func TestCompactBlock2(t *testing.T) {
...

	{
		...
		newColumnView, err := blk.GetColumnDataById(context.Background(), 3, true)
		require.NoError(t, err)
...

	{
		...
		testutil.ForEachBlock(rel, func(blk handle.Block) error {
			...
			case 0:
				...
				view, err := blk.GetColumnDataById(context.Background(), 0 /*uvw*/, true)
				require.NoError(t, err)
...
func TestAutoCompactABlk2(t *testing.T) {
	...
	doSearch := func(name string) func() {
		return func() {
			...
			for it.Valid() {
				blk := it.GetBlock()
				view, err := blk.GetColumnDataById(context.Background(), schema1.GetSingleSortKeyIdx(), true)
				assert.Nil(t, err)
				view.Close()
				it.Next()
			}
...
func TestMVCC1(t *testing.T) {
	...
	for it.Valid() {
		...
		if bid.BlockID == id.BlockID {
			view, err := block.GetColumnDataById(context.Background(), schema.GetSingleSortKeyIdx(), true)
			assert.Nil(t, err)
			defer view.Close()
			assert.Nil(t, view.DeleteMask)
...
func TestDelete1(t *testing.T) {
	...
	}
	{
		txn, rel := testutil.GetDefaultRelation(t, tae, schema.Name)
		blk := testutil.GetOneBlock(rel)
		view, err := blk.GetColumnDataById(context.Background(), schema.GetSingleSortKeyIdx(), true)
		assert.NoError(t, err)
		defer view.Close()
		assert.Nil(t, view.DeleteMask)
...
func TestMergeBlocks(t *testing.T) {
	...
	for it.Valid() {
		testutil.CheckAllColRowsByScan(t, rel, bat.Length(), false)
		col, err := it.GetBlock().GetMeta().(*catalog.BlockEntry).GetBlockData().GetColumnDataById(context.Background(), txn, schema, 0, true)
		assert.NoError(t, err)
		defer col.Close()
		t.Log(col)
		it.Next()
...
func TestGetColumnData(t *testing.T) {
	...
	txn, rel := tae.GetRelation()
	blk := testutil.GetOneBlock(rel)
	view, _ := blk.GetColumnDataById(context.Background(), 2, true)
	defer view.Close()
...
func TestCompactblk3(t *testing.T) {
	...
	processor.BlockFn = func(be *catalog.BlockEntry) error {
		...
		view, err := be.GetBlockData().GetColumnDataById(context.Background(), txn, schema, 0, true)
		assert.NoError(t, err)
		view.ApplyDeletes()
...
func TestDelete4(t *testing.T) {
	...
	scanFn := func() {
		...
		for it.Valid() {
			blk := it.GetBlock()
			view, err := blk.GetColumnDataById(context.Background(), 0, true)
			assert.NoError(t, err)
			defer view.Close()
			view.ApplyDeletes()
			if view.Length() != 0 {
