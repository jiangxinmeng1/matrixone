...
func TestHiddenWithPK1(t *testing.T) {
	...
	err := rel.Append(context.Background(), bats[0])
	{
		...
		for it.Valid() {
			blk := it.GetBlock()
			view, err := blk.GetColumnDataById(context.Background(), schema.PhyAddrKey.Idx, true)
...
func TestHidden2(t *testing.T) {
	...
	err := rel.Append(context.Background(), bats[0])
	{
		...
		for _, def := range schema.ColDefs {
			view, err := blk.GetColumnDataById(context.Background(), def.Idx, true)
			assert.NoError(t, err)
			defer view.Close()
	...
	txn, rel = testutil.GetDefaultRelation(t, tae, schema.Name)
	{
		...
		for it.Valid() {
			blk := it.GetBlock()
			hidden, err := blk.GetColumnDataById(context.Background(), schema.PhyAddrKey.Idx, true)
			assert.NoError(t, err)
			defer hidden.Close()
			hidden.ApplyDeletes()
			rows += hidden.Length()
